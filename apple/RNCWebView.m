...
static NSString *const NativeHandlerName = @"RNMSGHander";
static NSURLCredential* clientAuthenticationCredential;
static NSDictionary* customCertificatesForHost;
//H5调用方法名
static NSString *const PopToUp = @"popToUp";
static NSString *const PopToHome = @"popToHome";
static NSString *const ResetToLogin = @"resetToLogin";
static NSString *const ResetToAccount = @"resetToAccount";
#if !TARGET_OS_OSX
...
- (WKWebViewConfiguration *)setUpWkWebViewConfig
{
  ...
  //增加js调用消息
  [wkWebViewConfig.userContentController addScriptMessageHandler:[[RNCWeakScriptMessageDelegate alloc] initWithDelegate:self] name:NativeHandlerName];
  //增加注入代码,传递设备参数给H5
  NSString *getDeviceTypeHtml = [NSString stringWithFormat:
                                     @"window.%@ = {"
                                     ...
                                     "  }"
                                     "};",@"AppInvokeModel",@"ios",@""];
  WKUserScript *jsScript = [[WKUserScript alloc] initWithSource:getDeviceTypeHtml injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:YES];
  [wkWebViewConfig.userContentController addUserScript:jsScript];
...
- (void)removeFromSuperview
{
    if (_webView) {
        ...
        [_webView.configuration.userContentController removeScriptMessageHandlerForName:NativeHandlerName];
       ...
       didReceiveScriptMessage:(WKScriptMessage *)message
{
  ...
  } else if ([message.name isEqualToString:NativeHandlerName]) {
      if ([message.body isEqualToString:PopToUp]) {
          dispatch_async(dispatch_get_main_queue(), ^{
              [[NSNotificationCenter defaultCenter] postNotificationName:@"nativeCallReact" object:@{@"method":@"nativeEvent",@"messageId":@"popToUp"}];
          });
      }else if ([message.body isEqualToString:PopToHome]) {
          dispatch_async(dispatch_get_main_queue(), ^{
              [[NSNotificationCenter defaultCenter] postNotificationName:@"nativeCallReact" object:@{@"method":@"nativeEvent",@"messageId":@"popToHome"}];
          });
      }else if ([message.body isEqualToString:ResetToLogin]) {
          dispatch_async(dispatch_get_main_queue(), ^{
              [[NSNotificationCenter defaultCenter] postNotificationName:@"nativeCallReact" object:@{@"method":@"nativeEvent",@"messageId":@"resetToLogin"}];
          });
      }else if ([message.body isEqualToString:ResetToAccount]) {
          dispatch_async(dispatch_get_main_queue(), ^{
              [[NSNotificationCenter defaultCenter] postNotificationName:@"nativeCallReact" object:@{@"method":@"nativeEvent",@"messageId":@"resetToAccount"}];
          });
      }
  }
}
